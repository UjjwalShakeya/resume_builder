import { BriefcaseBusiness, Globe, icons, Linkedin, Mail, MapPin, Phone, User } from 'lucide-react'
...
const PersonalInfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
    ...
    // we have stored fields as an objects in array here
    const fields = [
    ...
    return (
        ...
        <div>
            ...
            {/* image icon and remove background button here */}
            ...
            {fields.map((field) => {
                ...
                return (
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        <input type={field.type} value={data[field.key] || ""} onChange={(e) => handleChange(field.key, e.target.value)} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors' placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required} />
                    </div>
                )
            }
            )}
